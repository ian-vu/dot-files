...
function parseProgressModelSpec(
  value: string,
): ProgressModelPreference | undefined {
...
function settingsModelValue(
  ...
  if (
    progressSection &&
    typeof progressSection === "object" &&
    !Array.isArray(progressSection)
  ) {
...
function resolveProgressModelPreference(
  cwd: string,
): ProgressModelPreference | undefined {
...
async function getFastProgressModelAuth(
  ...
  for (const candidate of FAST_PROGRESS_MODELS) {
    if (
      configuredModel &&
      formatProgressModelKey(candidate) ===
        formatProgressModelKey(configuredModel)
    ) {
      continue;
    }
...
const ERROR_LINE_PATTERN =
  /(error|failed|TS\d+|E[A-Z]+\d*|Command exited|denied|refused|aborted|panic|fatal|exception)/i;
...
function errorSummary(text: string | undefined): string | undefined {
  ...
  const matched =
    lines.find((line) => ERROR_LINE_PATTERN.test(line)) ??
...
class ProgressFactCollector {
  ...
  recordAssistantUpdate(message: unknown): ProgressActivity | undefined {
    ...
    const text = extractTextContent(
      record.content as readonly unknown[] | undefined,
    );
    if (!text) return undefined;
  ...
  recordMessageEnd(
    message: unknown,
  ): ProgressActivity | "emptyFinalStop" | "ignored" {
    ...
    if (stopReason === "stop") {
      const finalText = extractTextContent(
        record.content as readonly unknown[] | undefined,
      );
      ...
      const error =
        typeof record.errorMessage === "string"
          ? compactText(record.errorMessage)
          : undefined;
      text = error
...
export class FooterProgressEngine {
  ...
  private pump(ctx: ExtensionContext): void {
    ...
    if (
      ...
      job.activityIndex <= this.latestAcceptedActivityIndex
    ) {
      this.pump(ctx);
      return;
    }
  ...
  private considerDisplayingCheckpoint(
    ...
    this.displayTimer = setTimeout(() => {
      ...
      if (
        !pending ||
        pending.activityIndex !== this.facts.latestActivityIndex()
      )
        return;
...
function isNearDuplicateProgress(current: string, previous: string): boolean {
  ...
  return (
    normalizedActionFragment(current) === normalizedActionFragment(previous)
  );
}
