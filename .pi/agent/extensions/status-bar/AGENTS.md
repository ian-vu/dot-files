...
- `costs/daily-cost.ts` — live daily/week cost runtime state restore/persist using per-day, per-session files.
...
- Keep runtime daily/week cost state out of committed/stowed config; it belongs under XDG state unless intentionally redesigned.
- Daily/week cost totals are derived from per-date folders with one JSON file per session to avoid cross-session write contention.
